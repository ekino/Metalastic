...
import com.metalastic.processor.collecting.withStarProjectedGenericArguments
 ...
 */
class MetaClassModelClassDeclaration(val model: MetalasticGraph.MetaClassModel) :
  ...
  override fun asStarProjectedType(): KSType =
    MetaClassModelKSType(this).withStarProjectedGenericArguments()

  override fun asType(typeArguments: List<KSTypeArgument>): KSType =
    if (typeArguments.isEmpty()) MetaClassModelKSType(this)
    else
  ...
  override val packageName: KSName = MetaClassModelKSName(model.packageName)

  override val parentDeclaration: KSDeclaration? =
    model.parentModel?.let { MetaClassModelClassDeclaration(it) }

  override val qualifiedName: KSName = MetaClassModelKSName(model.fullyQualifiedName)

  override val simpleName: KSName = MetaClassModelKSName(model.qClassName)
  ...
  override fun <D, R> accept(visitor: KSVisitor<D, R>, data: D): R {
    throw UnsupportedOperationException("MetaClassModelClassDeclaration does not support visitors")
  }
  ...
  override fun equals(other: Any?): Boolean {
    ...
    if (other !is MetaClassModelClassDeclaration) return false
  ...
  override fun toString(): String = "MetaClassModelClassDeclaration(${model.fullyQualifiedName})"
}
