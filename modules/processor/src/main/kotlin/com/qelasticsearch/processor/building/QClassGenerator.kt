...
import com.qelasticsearch.processor.CoreConstants.DocumentClass.INDEX_NAME_CONSTANT
import com.qelasticsearch.processor.CoreConstants.PRODUCT_NAME
import com.qelasticsearch.processor.QElasticsearchSymbolProcessor
import com.qelasticsearch.processor.collecting.fullyQualifiedName
import com.qelasticsearch.processor.collecting.toSafeTypeName
import com.qelasticsearch.processor.kspimplementation.QClassModelClassDeclaration
...
class QClassGenerator(
  private val rootModel: ElasticsearchGraph.QClassModel,
  ...
  companion object {
    val SYMBOL_PROCESSOR_FQN = QElasticsearchSymbolProcessor::class.qualifiedName!!
    val documentClass =
  ...
  private fun buildTypeSpec(model: ElasticsearchGraph.QClassModel): TypeSpec {
    ...
    when (model) {
      is ElasticsearchGraph.DocumentClass -> {
      ...
      is ElasticsearchGraph.ObjectClass -> {
        // Add generic type parameter T
    ...
    if (model is ElasticsearchGraph.DocumentClass) {
  ...
  private fun generateObjectFieldProperty(field: ObjectFieldModel): PropertySpec {
    ...
    val qClassKsClassDeclaration = QClassModelClassDeclaration(field.targetModel)
    val sourceTypeName = field.type.toSafeTypeName(typeParameterResolver)
  ...
  private fun generateCompanionObject(document: ElasticsearchGraph.DocumentClass): TypeSpec {
    val companionBuilder = TypeSpec.companionObjectBuilder()

    // Build the ClassName for the Q-class
    val qClassName = QClassModelClassDeclaration(document).toClassName()
  ...
  private fun generateDocumentKdoc(document: ElasticsearchGraph.DocumentClass): String {
    return """
  ...
  private fun generateObjectFieldKdoc(objectModel: ElasticsearchGraph.QClassModel): String {
  ...
  private fun generateCompanionPropertyKdoc(document: ElasticsearchGraph.DocumentClass): String {
    return """
