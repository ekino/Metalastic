...
class GraphBuilder(val resolver: Resolver) {
  ...
  private fun Collection<KSClassDeclaration>.toGraphWithoutFields(): ElasticsearchGraph {
    val graphWithoutFields = ElasticsearchGraph()
  ...
  private fun ElasticsearchGraph.linkModelsAndPopulateFields(): ElasticsearchGraph {
    val modelsByNestingOrder: MutableList<ElasticsearchGraph.QClassModel> =
      (documentModels().filterNot { it.isNested } + objectModels().filterNot { it.isNested })
        .toMutableList()

    val finalGraph = ElasticsearchGraph()
    generateSequence {
