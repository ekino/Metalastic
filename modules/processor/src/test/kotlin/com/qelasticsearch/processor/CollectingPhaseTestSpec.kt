...
import com.qelasticsearch.processor.testing.ProcessorTestBuilder
import com.qelasticsearch.processor.testing.TestDataSources
...
/**
 * Tests for the enhanced collecting phase using the new unified QElasticsearchModel approach.
 *
 * This test validates:
...
class CollectingPhaseTestSpec :
  ShouldSpec({
    should("successfully collect simple document with all fields") {
      val graph =
        ProcessorTestBuilder.withSource(
          ...
          .testElasticsearchGraph()

      graph.documentModels().shouldHaveSize(1)

      val documentModel = graph.documentModels().first()
      documentModel.shouldBeInstanceOf<ElasticsearchGraph.DocumentClass> {
    ...
    should("successfully collect complex document with multiple nesting levels") {
      val graph =
        ProcessorTestBuilder.withSource(TestDataSources.complexDocument()).testElasticsearchGraph()
    ...
    should("handle MultiField annotations correctly with enhanced field models") {
      val graph =
        ProcessorTestBuilder.withSource(TestDataSources.multiFieldDocument())
          .testElasticsearchGraph()
    ...
    should("handle inner class annotated with @Document correctly") {
      val graph =
        ProcessorTestBuilder.withSource(TestDataSources.outerDocument()).testElasticsearchGraph()
      ...
      outerDocumentModel.shouldNotBeNull {
        this.shouldBeInstanceOf<ElasticsearchGraph.DocumentClass> {
      ...
      innerDocumentModel.shouldNotBeNull {
        this.shouldBeInstanceOf<ElasticsearchGraph.DocumentClass> {
          ...
          it.nestedClasses().singleOrNull() shouldNotBeNull
            {
              shouldBeInstanceOf<ElasticsearchGraph.ObjectClass> { nestedObject ->
    ...
    should("test TestDocument field collecting") {
      val graph =
        ProcessorTestBuilder.withSources(TestDataSources.testDocument()).testElasticsearchGraph()
    ...
    should("test IndexPerson field collecting") {
      val graph =
        ProcessorTestBuilder.withSources(TestDataSources.indexPerson()).testElasticsearchGraph()
    ...
    should("test JavaTestDocument field collecting") {
      val graph =
        ProcessorTestBuilder.withSources(TestDataSources.javaTestDocument())
          .testElasticsearchGraph()
