 ...
 */
class NestedClassProcessor(private val logger: KSPLogger, private val codeGenUtils: CodeGenerationUtils) {
    private val globalObjectFields = mutableMapOf<String, ObjectFieldInfo>()

    /**
     * Collects all object fields from a document class.
     */
    fun collectObjectFields(documentClass: KSClassDeclaration, fieldTypeExtractor: FieldTypeExtractor) {
     ...
     */
    fun getNestedClassesForDocument(documentClass: KSClassDeclaration): List<ObjectFieldInfo> = globalObjectFields.values.filter { objectFieldInfo ->
     ...
     */
    private fun getNestedClassesForClass(parentClass: KSClassDeclaration): List<ObjectFieldInfo> = globalObjectFields.values.filter { objectFieldInfo ->
        isDirectNestedClass(objectFieldInfo.classDeclaration, parentClass)
    }

    private fun isDirectNestedClass(nestedClass: KSClassDeclaration, parentClass: KSClassDeclaration): Boolean {
    ...
    private fun registerObjectField(classDeclaration: KSClassDeclaration, documentClass: KSClassDeclaration?) {
    ...
    private fun collectObjectFieldsFromClass(classDeclaration: KSClassDeclaration, fieldTypeExtractor: FieldTypeExtractor) {
    ...
    private fun isNestedWithinClass(nestedClass: KSClassDeclaration, parentClass: KSClassDeclaration): Boolean {
     ...
     */
    private fun processUniqueProperties(classDeclaration: KSClassDeclaration, onProperty: (KSPropertyDeclaration) -> Unit) {
