...
class QElasticsearchSymbolProcessor(
     ...
     * Avoids duplication between getFieldDelegate() and getFieldClass().
    ...
    private fun addPropertiesToObjectBuilder(
        ...
        // Add nested objects for classes that are nested within this document
        addNestedObjectsToBuilder(documentClass, objectBuilder)
    }

    /**
     * Adds nested objects to the main document Q-class for nested classes within the document.
     */
    private fun addNestedObjectsToBuilder(
        documentClass: KSClassDeclaration,
        ...
        nestedClasses.forEach { objectFieldInfo ->
            val nestedObjectBuilder = createNestedObjectBuilder(objectFieldInfo)
     ...
     */
    private fun createNestedObjectBuilder(objectFieldInfo: ObjectFieldInfo): TypeSpec.Builder {
        ...
        addPropertiesFromClassToObjectBuilder(objectFieldInfo.classDeclaration, objectBuilder)

        return objectBuilder
    }

    /**
     * Adds properties from a class declaration to an object builder (for nested objects).
     */
    private fun addPropertiesFromClassToObjectBuilder(
        ...
        // Recursively add nested objects for any nested classes within this nested class
        addNestedObjectsToBuilder(classDeclaration, objectBuilder)
    }
    ...
    private fun generateSimpleFieldProperty(context: FieldGenerationContext) {
        ...
        when (context.fieldType.elasticsearchType) {
            FieldType.Object, FieldType.Nested -> {
                ...
                return generateObjectFieldProperty(
    ...
    private fun generateObjectFieldProperty(
        ...
        // Get the current document class context to check if this is a nested class within the same document
        val currentDocumentClass = findContainingDocumentClass(property)

        // A class is only considered nested in current document if:
        // 1. It has a parent document class that matches the current document
        // 2. It's actually declared as a nested class within that document
        // 3. It's not an external class imported from elsewhere
        // 4. The qualified name shows it's truly nested (contains the parent class name)
        val isNestedInCurrentDocument =
        ...
        // Determine the reference name for delegate calls
        val finalReferenceClassName =
            if (isNestedInCurrentDocument) {
                // For nested classes within the current document, use simple class name (no Q prefix)
                objectFieldInfo.classDeclaration.simpleName.asString()
            } else if (objectFieldInfo.parentDocumentClass != null) {
                // For nested classes within other Q-classes, use ParentQClass.NestedClass format
                ...
                // For external classes, always ensure Q prefix
        ...
        // For nested classes within current document, use simple ClassName without package (resolves to nested object)
        // For nested classes within other Q-classes, use nested ClassName properly
        // For external classes, use full ClassName with package
        val propertyTypeName =
            if (isNestedInCurrentDocument) {
                // For nested classes in same document, create a simple ClassName that doesn't generate imports
                ClassName("", finalReferenceClassName)
            } else if (objectFieldInfo.parentDocumentClass != null) {
                // For nested classes in other Q-classes, create proper nested ClassName
                val parentQClassName = generateUniqueQClassName(objectFieldInfo.parentDocumentClass)
                val nestedClassName = objectFieldInfo.classDeclaration.simpleName.asString()
                ClassName(targetPackage, parentQClassName).nestedClass(nestedClassName)
            } else {
                ClassName(targetPackage, finalReferenceClassName) // Full package for external Q-classes
            }

        // Create proper delegate call with correct class reference
        val delegateCall =
            if (isNested) {
    ...
    private data class ObjectFieldInfo(
        ...
        val parentDocumentClass: KSClassDeclaration? = null, // If nested within a document class
    )
    ...
    /**
     * Checks if a class is actually declared as a nested class within another class.
     */
    private fun isActuallyNestedClass(
        ...
        // Check if the nested class is directly declared within the parent class
        ...
        documentClass.getAllProperties().forEach { property ->
            ...
            if (fieldType.isObjectType) {
                ...
                if (nestedClass != null) {
                    ...
                    // Determine parent document class - prioritize direct nesting within current document
                    val parentDocumentClass =
                        if (isActuallyNestedClass(nestedClass, documentClass)) {
                            // If nested within current document, set current document as parent
                            documentClass
                        } else {
                            // Otherwise, use the general nested class detection
                            isNestedWithinAnyQClass(nestedClass)
                        }
        ...
        try {
            ...
            filteredClasses.forEach { classDeclaration ->
                ...
                if (shouldRegister) {
                    ...
                    val parentDocumentClass = isNestedWithinAnyQClass(classDeclaration)
                    globalObjectFields[objectFieldKey] =
                        ObjectFieldInfo(
            ...
            if (fieldType.isObjectType) {
                ...
                if (nestedClass != null) {
                    ...
                    if (objectFieldKey !in globalObjectFields) {
                        val parentDocumentClass = isNestedWithinAnyQClass(nestedClass)
                        globalObjectFields[objectFieldKey] =
                            ObjectFieldInfo(
                                className = className,
                                packageName = targetPackage,
    ...
    private fun generateAllObjectFields() {
        ...
        objectFieldsToGenerate.forEach { objectFieldInfo ->
            // Skip nested classes that should be included as nested objects in main document classes
            if (objectFieldInfo.parentDocumentClass != null) {
                return@forEach
            }
    ...
    private fun generateObjectFieldsClassFromInfo(objectFieldInfo: ObjectFieldInfo): FileSpec {
        ...
        // Add nested objects for classes that are nested within this class
        addNestedObjectsToBuilder(objectFieldInfo.classDeclaration, objectBuilder)
    ...
    private fun ensureObjectFieldRegistered(fieldType: ProcessedFieldType) {
        ...
        val parentDocumentClass = isNestedWithinAnyQClass(actualClassDeclaration)
    ...
    /**
     * Collects all nested classes that should be included as nested objects in a document Q-class.
     */
    private fun getNestedClassesForDocument(documentClass: KSClassDeclaration): List<ObjectFieldInfo> =
        globalObjectFields.values.filter { objectFieldInfo ->
            objectFieldInfo.parentDocumentClass == documentClass
        }

    /**
     * Determines if a class is nested within any parent class that will have a Q-class generated.
     * This includes both @Document classes and classes referenced by Object/Nested fields.
     */
    private fun isNestedWithinAnyQClass(classDeclaration: KSClassDeclaration): KSClassDeclaration? {
        ...
        while (parentClass != null) {
            if (parentClass is KSClassDeclaration) {
                // Check if this parent class has @Document annotation or will have a Q-class generated
    ...
    /**
     * Finds the containing document class for a property.
     */
    private fun findContainingDocumentClass(property: KSPropertyDeclaration): KSClassDeclaration? {
        ...
        while (currentDeclaration != null) {
            if (currentDeclaration is KSClassDeclaration) {
                // Check if this class has @Document annotation
                val hasDocumentAnnotation =
