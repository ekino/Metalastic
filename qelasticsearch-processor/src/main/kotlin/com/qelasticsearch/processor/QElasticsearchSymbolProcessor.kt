...
/**
 * Refactored KSP-based annotation processor that generates type-safe Elasticsearch DSL classes
 * from Spring Data Elasticsearch @Document annotated classes.
 *
 */
class QElasticsearchSymbolProcessor(private val codeGenerator: CodeGenerator, private val logger: KSPLogger) : SymbolProcessor {
    ...
    private fun processDocumentClass(documentClass: KSClassDeclaration, objectFieldRegistry: ObjectFieldRegistry) {
        ...
        val qIndexClass = generateQIndexClass(documentClass, indexName, className, packageName, objectFieldRegistry)
    ...
    private fun generateAllObjectFields(globalObjectFields: Map<String, ObjectFieldInfo>, objectFieldRegistry: ObjectFieldRegistry) {
        val objectFieldsToGenerate =
            globalObjectFields.values.filter {
                it.parentDocumentClass == null
            }
    ...
    private fun generateObjectFieldsClassFromInfo(objectFieldInfo: ObjectFieldInfo, objectFieldRegistry: ObjectFieldRegistry): FileSpec {
        val importContext = ImportContext()
    ...
    private fun createFileSpec(packageName: String, qIndexClassName: String, objectBuilder: TypeSpec.Builder, importContext: ImportContext): FileSpec {
        val fileBuilder =
            FileSpec
    ...
    private fun createObjectFieldFileSpec(objectFieldInfo: ObjectFieldInfo, objectBuilder: TypeSpec.Builder, importContext: ImportContext): FileSpec {
        val fileBuilder =
            FileSpec
    ...
    private fun KSClassDeclaration.findAnnotation(annotationClass: kotlin.reflect.KClass<*>): com.google.devtools.ksp.symbol.KSAnnotation? = annotations.find {
        it.annotationType
            .resolve()
    ...
    private inline fun <reified T> com.google.devtools.ksp.symbol.KSAnnotation.getArgumentValue(name: String): T? =
